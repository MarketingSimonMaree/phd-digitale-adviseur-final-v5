...
export default function InteractiveAvatar({ children }: Props) {
  ...
  const handleInterrupt = async (): Promise<void> => {
    ...
    try {
      ...
      await (avatar.current as any).interrupt();
  ...
  async function startSession(): Promise<void> {
    ...
    try {
      ...
      // Start voice chat (we gebruiken altijd dezelfde startVoiceChat aanroep)
      setTimeout(async () => {
        try {
          if (avatar.current) {
  ...
  function setupAvatarEventListeners(): void {
    ...
    avatar.current.on(StreamingEvents.USER_TALKING_MESSAGE, (event: any) => {
      ...
      if (event.detail?.message) {
        ...
        else if (chatMode === 'voice_mode') {
          if (isMicrophoneEnabled) {
            // Als microfoon AAN staat: voeg bericht toe en log naar DB
            ...
            // Als microfoon UIT staat: onderbreek de avatar om reactie te voorkomen
  ...
  // Functie om de microfoon in/uit te schakelen - SUPER SIMPEL
  const toggleMicrophone = (): void => {
    // Toggle de state direct 
    const newStatus = !isMicrophoneEnabled;
    
    // Onderbreek huidige spraak wanneer we de microfoon uitzetten
    if (!newStatus && avatar.current) {
      handleInterrupt();
    }
    ...
    // UI feedback - toon even een melding
    setDebug(newStatus 
